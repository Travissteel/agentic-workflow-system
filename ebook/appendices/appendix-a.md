# Appendix A: Complete Orchestrator Prompt Template

## Introduction

This appendix contains the complete, production-ready orchestrator template that serves as the "brain" of your DOE Framework implementation. This is the exact file that Antigravity IDE reads when acting as your master orchestrator.

**What this template is**: The `GEMINI.md` file is a comprehensive instruction set that transforms Antigravity IDE into a sophisticated project manager. It defines the orchestrator's role, responsibilities, workflows, and rules for delegating work to specialized subagents.

**Why it matters**: This single file is the foundation of your entire agentic workflow system. It teaches Antigravity IDE how to:
- Break down complex projects into manageable tasks
- Delegate work to specialized subagents (coder, tester, deployer)
- Maintain project state across 200k token context
- Handle errors with human-in-the-loop escalation
- Deploy workflows to production

**How to use it**: Copy this entire template to your project's `.antigravity/GEMINI.md` file. The Antigravity IDE extension automatically reads this file and uses it as the primary instruction set. You can customize sections marked with `[CUSTOMIZE]` comments, but the core framework should remain intact.

---

## The Complete Template

Below is the full orchestrator template. Copy everything from the start of the code block to the end.

```markdown
# YOU ARE THE ORCHESTRATOR

You are Antigravity IDE with a 200k context window, and you ARE the orchestration system based on **Nick Saraev's Directive Orchestration Execution (DOE) Framework**. You manage the entire project, create todo lists, and delegate individual tasks to specialized subagents.
...
1. **IDE Setup**: Using VS Code with Antigravity IDE extension
...
6. **[CUSTOMIZE]** Add your project-specific rules here (e.g., "Always create pages for EVERY link in headers/footers")
7. **[CUSTOMIZE]** Add your task splitting rules here (e.g., "Split large data tasks - Don't ask coder to create 50+ items at once")
...
5. **[CUSTOMIZE]** Add your project-specific prohibitions here
6. **[CUSTOMIZE]** Add your token limit rules here
...
**[CUSTOMIZE]** Add your project-specific task splitting examples here.
...
- **[CUSTOMIZE]** Add your project-specific mistakes here
...
- **[CUSTOMIZE]** Add your project-specific success criteria here
...
## Project Status: [CUSTOMIZE - Your Project Name]

**[CUSTOMIZE]** Replace this section with your project-specific status tracking.

**Current State:** Phase X Complete

- **[Component 1]**: Status and details
- **[Component 2]**: Status and details
- **[Component 3]**: Status and details
...
Now that you have the complete template, here's how to customize it for your specific business needs.

### Parts to Customize

Look for `[CUSTOMIZE]` markers throughout the template. These indicate sections you should adapt to your project:

**1. Project-Specific Rules (Lines 420-426)**

Add rules specific to your domain. Examples:
...
**YOU (the orchestrator) MUST:**
...
6. **ALWAYS verify all API endpoints return proper error codes** - No silent failures!
7. **Split database operations into batches of 100 records max** - Prevent timeouts
8. **Create automated tests for all payment flows** - Financial accuracy is critical
```

**2. Task Splitting Guidelines (Lines 433-445)**

Add examples from your specific workflows:

```markdown
## Task Splitting Guidelines

**BAD**: "Migrate all 1000 customer records to new CRM"
**GOOD**: Split into batches:
- "Set up migration script with error handling"
- "Test migration with 10 sample records"
- "Migrate first 250 customer records"
- "Migrate next 250 customer records"
- "Verify all data integrity and relationships"
```

**3. Common Mistakes Section (Lines 559-567)**

Document mistakes you've encountered:

```markdown
## Common Mistakes to Avoid

...
- **Not validating email addresses before sending** (causes bounce issues)
- **Hardcoding API keys in code** (security vulnerability)
- **Not testing with edge cases** (empty strings, null values, special characters)
```

**4. Success Criteria (Lines 569-577)**

Define what success looks like for your projects:

```markdown
## Success Looks Like

...
- **All payment flows tested in Stripe test mode** before production
- **Database migrations include rollback scripts** for safety
- **API documentation generated automatically** from code comments
```

**5. Project Status Section (Lines 611-622)**

Replace with your actual project tracking:

```markdown
## Project Status: Customer Portal v2.0

**Current State:** Phase 3 - Payment Integration

- **Authentication**: OAuth2 flow complete, JWT refresh working
- **Dashboard**: User analytics displaying correctly
- **Payments**: Stripe integration in progress (test mode)
- **Next Steps**: Complete payment flow, add invoice generation
```

### Parts to Leave Unchanged

These are core framework components. Do NOT modify unless you deeply understand the DOE framework:

1. **The 5-Phase DOE Workflow** (Lines 26-353)
   - This is the backbone of the framework
   - Changing this breaks the orchestration pattern

2. **Available Subagents** (Lines 355-412)
   - Standard subagent definitions
   - Only add NEW subagents, don't modify existing ones

3. **Orchestration Flow** (Lines 478-513)
   - This defines how work flows through the system
   - Critical for proper delegation

4. **DOE Terminology Reference** (Lines 526-537)
   - Standard framework vocabulary
   - Ensures consistent communication

5. **DOE Framework Attribution** (Lines 629-635)
   - Credit to Nick Saraev's original work
...
**YOU (the orchestrator) MUST:**
...
6. **ALWAYS test checkout flows with multiple payment methods** - Card, PayPal, Apple Pay
7. **Verify inventory counts before allowing purchase** - Prevent overselling
8. **Create orders in "pending" state until payment confirms** - Handle payment failures gracefully
```
...
**YOU (the orchestrator) MUST:**
...
6. **ALWAYS implement feature flags for new functionality** - Allow gradual rollouts
7. **Test with multiple subscription tiers** - Free, Pro, Enterprise
8. **Monitor API rate limits and implement queuing** - Prevent service disruptions
```
...
**YOU (the orchestrator) MUST:**
...
6. **ALWAYS sanitize user input before storing** - Prevent XSS attacks
7. **Create drafts before publishing** - Allow content review
8. **Implement version history for all content** - Allow rollbacks
```
...
2. **Copy the template**:
   - Copy the entire template from the code block above
   - Save it as `.antigravity/GEMINI.md` in your project root

3. **Customize for your project**:
   - Search for `[CUSTOMIZE]` markers
   - Replace with your project-specific content
   - Keep core framework sections intact

4. **Verify the agent reads it**:
   - Open a new Antigravity IDE chat
   - Type: "What are your orchestrator rules?"
   - The agent should reference this file

### File Location

The orchestrator template MUST be located at:

```
your-project/
└── .antigravity/
    └── GEMINI.md    ← Place the template here
```

**Why this location matters**:
- Antigravity IDE automatically searches for `.antigravity/GEMINI.md`
- This is the first file the agent reads when starting a new conversation
- It establishes the orchestrator's role and behavior

### When the Orchestrator Uses This File

Antigravity IDE reads `GEMINI.md` in these scenarios:

1. **New conversation start**: Loads the complete instruction set
2. **Project context switching**: Re-reads when you switch projects
3. **After framework updates**: Picks up new rules automatically

### When to Update This File

You should update `GEMINI.md` when:

**Self-Annealing Learnings**:
- Agents discover edge cases during execution
- New error patterns emerge that need documentation
- Better task splitting strategies are found

```markdown
## Common Mistakes to Avoid

- **NEW**: Not checking for null values in API responses (caused 3 failures on 2026-02-10)
```

**Project Requirements Change**:
- New tools or MCPs are added to the stack
- Business rules change (e.g., new compliance requirements)
- Team processes evolve

**Framework Improvements**:
- New DOE framework patterns are released
- Better orchestration strategies are discovered
...
1. **Document the learning**:
   ```markdown
   ## Lessons Learned (Self-Annealing)

   ### 2026-02-13: API Timeout Handling
   - **Problem**: Large file uploads timed out after 30 seconds
   - **Solution**: Split uploads into 5MB chunks with retry logic
   - **New Rule**: Always chunk file uploads larger than 5MB
   ```

2. **Update relevant sections**:
   - Add to "Common Mistakes" if it's an error pattern
   - Add to "Task Splitting Guidelines" if it's about complexity
   - Add to "CRITICAL RULES" if it's a must-follow requirement

3. **Test the update**:
   - Start a new Antigravity IDE conversation
   - Verify the agent follows the new rule
   - If ignored, make the rule more prominent
...
**ALWAYS commit GEMINI.md to git**:

```bash
git add .antigravity/GEMINI.md
git commit -m "Add orchestrator template"
git push
```

**Why this matters**:
- Team members get the same orchestrator behavior
- Changes are tracked and can be rolled back
- Self-annealing learnings are preserved across environments

**Gitignore considerations**:
- DO commit `.antigravity/GEMINI.md` (orchestrator rules)
- DO commit `.antigravity/agents/*.md` (subagent definitions)
- DON'T commit `.env` files (credentials)
- DON'T commit temporary execution outputs

### Incorporating Framework Updates

The DOE framework evolves over time. To update your orchestrator:

1. **Check for new versions**:
   - Visit the resource site (see callout below)
   - Review the changelog
   - Identify what changed

2. **Backup your customizations**:
   ```bash
   cp .antigravity/GEMINI.md .antigravity/GEMINI.md.backup
   ```

3. **Merge new features**:
   - Copy new sections from the updated template
   - Preserve your `[CUSTOMIZE]` sections
...
4. **Document the upgrade**:
   ...
   - v3.1 (2026-02-13): Added Shadow Orchestrator pattern
   - v3.0 (2026-01-15): Added Hybrid Wrapper Strategy
   - v2.5 (2025-12-01): Initial DOE framework implementation
   ```

### Testing Your Orchestrator

After setting up or updating `GEMINI.md`, verify it works correctly:

**Test 1: Rule Recognition**
```
You: "What are your critical rules?"
Agent: Should list rules from "CRITICAL RULES FOR YOU" section
```

**Test 2: Workflow Understanding**
```
You: "Explain the 5 phases of the DOE workflow"
Agent: Should describe all 5 phases accurately
```

**Test 3: Subagent Awareness**
```
You: "When do you invoke the coder subagent?"
Agent: Should explain delegation for coding tasks
```

**Test 4: Custom Rules**
```
You: "What are the project-specific rules?"
Agent: Should reference your [CUSTOMIZE] additions
```

### Common Issues and Solutions

**Issue**: The agent doesn't follow the orchestrator rules

**Solution**: Check that:
- File is named exactly `GEMINI.md` (case-sensitive)
- File is in `.antigravity/` directory at project root
- File has proper markdown formatting
- You've started a NEW conversation after adding the file

**Issue**: Orchestrator rules conflict with subagent rules

**Solution**: Remember the hierarchy:
1. Orchestrator (GEMINI.md) = Master rules
2. Subagents (.antigravity/agents/*.md) = Specialized rules
3. Subagent rules should EXTEND orchestrator, not contradict

**Issue**: Custom rules are being ignored

**Solution**: Make them more prominent:
- Move to "CRITICAL RULES FOR YOU" section
- Use bold formatting: `**ALWAYS do this**`
- Add to "YOU MUST NEVER" section for prohibitions
- Include in "Common Mistakes" with examples

---

> **DOWNLOAD THIS TEMPLATE**
>
> Get the latest version of this orchestrator template and all other resources at:
>
...
1. **Copy the template** from this appendix to `.antigravity/GEMINI.md`
2. **Customize the marked sections** with your project-specific rules
3. **Set up subagents** (see Appendix B for agent definitions)
4. **Test the orchestrator** with a simple project
5. **Document learnings** as you use the system
6. **Share with your team** via git

The orchestrator is now ready to manage your agentic workflows. In the next appendix, we'll cover the subagent definitions that work alongside this orchestrator.
